...
<head>
    ...
    <script>
        const MALL_pos = [[439, 1177], [830, 1213], [649, 965], [2193, 1045]];
        const EB_pos = [[1900, 1204], [1905, 1024], [1558, 1114], [1571, 271], [1630, 487], [2300, 487]];
...
<body class="body_DNG">
    ...
    <script>
        ...
        function draw_onsite(width_Img, height_Img, bodyWidth, bodyHeight)
        {
            ...
            document.getElementById("OUT_square_11").style.top = normalize(706, height_Img, bodyHeight) + 'px'; 
            document.getElementById("OUT_square_11").style.left = normalize(32, width_Img, bodyWidth) + 'px'; 
        ...
        function draw_MALL(width_Img, height_Img, bodyWidth, bodyHeight)
        {
            for(var sensor = 1; sensor <= MALL_pos.length; sensor++)
            {
                ...
                document.getElementById("MALL_lo_" + sensor).style.top = normalize(525, height_Img, bodyHeight) + 'px';
        ...
        {   
            for(var sensor = 1; sensor <= EB_pos.length; sensor++)
            {
                ...
                { 
                    ...
                    document.getElementById("EB_lo_" + sensor).style.top = normalize(100, height_Img, bodyHeight) + 'px';
                    document.getElementById("EB_lo_" + sensor).style.left = normalize(752 + sensor * (18 + 100), width_Img, bodyWidth) + 'px';
                ...
                else
                {
                    document.getElementById("EB_lo_" + sensor).style.width = normalize(100, width_Img, bodyWidth) + 'px';
                    document.getElementById("EB_lo_" + sensor).style.height = normalize(140, height_Img, bodyHeight) + 'px';
                    document.getElementById("EB_lo_" + sensor).style.top = normalize(220, height_Img, bodyHeight) + 'px';
                    document.getElementById("EB_lo_" + sensor).style.left = normalize(398 + sensor * (18 + 100), width_Img, bodyWidth) + 'px';
                    document.getElementById("EB_lo_" + sensor).innerHTML = "Lo." + sensor + "<br>" + Math.floor(sensor_value / 10)  + "." + sensor_value % 10;            
                    document.getElementById("EB_lo_" + sensor).classList.add("line-visible");                    
                }
